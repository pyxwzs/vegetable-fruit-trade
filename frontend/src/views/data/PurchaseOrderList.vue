<template>
  <div class="purchase-list">
    ...
    <!-- 新建 -->
    <el-dialog v-model="createVisible" title="新建采购单" width="620px" destroy-on-close>
    ...
    <!-- 详情 -->
    <el-dialog v-model="detailVisible" title="采购单详情" width="680px" destroy-on-close>
...
const createForm = ref({ supplierId: null, remark: '', items: [{ productId: null, quantity: 1, price: 1 }] })
...
const openCreate = () => {
  createForm.value = { supplierId: supplierOptions.value[0]?.id ?? null, remark: '', items: [{ productId: null, quantity: 1, price: 1 }] }
...
const submitCreate = async () => {
  ...
  if (!f.supplierId || !f.items?.length) { ElMessage.warning('请选择供应商并添加明细'); return }
  ...
  try {
    await createPurchaseOrder({
      supplierId: f.supplierId, remark: f.remark || undefined,
...
const doCancel = (row) => {
  ElMessageBox.confirm(`取消订单「${row.orderNo}」？`, '提示', { type: 'warning' })
    .then(async () => { await cancelPurchaseOrder(row.id); ElMessage.success('已取消'); loadData() })
    .catch(() => {})
}
