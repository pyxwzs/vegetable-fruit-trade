  ...
  <div class="purchase-list">
    ...
    <el-dialog v-model="detailVisible" title="采购单详情" width="720px" destroy-on-close>
      ...
      <template #footer>
        ...
        <el-button v-if="detailOrder && canPrint(detailOrder)" @click="printOrder">打印单据</el-button>
        <el-button
...
const canPrint = (o) => o && ['APPROVED', 'SHIPPED', 'RECEIVED', 'COMPLETED'].includes(o.status)
...
const printOrder = () => {
  ...
  const rows = (o.items || [])
    ...
    .join('')
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>采购单 ${o.orderNo}</title>
    ...
    <h2>采购单</h2><p>单号：${o.orderNo}</p><p>供应商：${o.supplier?.name || ''}</p>
