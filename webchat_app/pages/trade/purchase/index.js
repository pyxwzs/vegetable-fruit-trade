...
const FILTERS = [
  ...
  { label: '入库', value: 'COMPLETED' },
  { label: '未入库', value: 'PENDING' },
];

function mapOrder(row) {
  const unpaid = calcUnpaid(row.totalAmount, row.paidAmount);
  return {
    id: row.id,
    ...
    paidAmountText: fmtMoney(row.paidAmount),
    unpaid,
    unpaidText: fmtMoney(unpaid),
    status: row.status,
    statusText: PURCHASE_STATUS[row.status] || row.status,
    statusTheme: purchaseStatusTheme(row.status),
    ...
    canPay: row.status !== 'CANCELLED' && row.paymentStatus !== 'PAID',
...
Page({
  ...
  async loadGlobalBalance() {
    try {
      ...
      const data = res.data || {};
      this.setData({
        unpaidTotal: Number(data.unpaidToFarmers || 0),
        unpaidTotalText: fmtMoney(data.unpaidToFarmers),
      });
    } catch {
  ...
  async loadSummary() {
    ...
    try {
      ...
      const ordersRes = await getPurchaseOrders(params);
  ...
  onCardTap(e) {
    ...
    if (id) wx.navigateTo({ url: `/pages/trade/purchase/detail?id=${id}` });
  },
    ...
    wx.showModal({
      ...
      success: async (res) => {
        ...
        try {
          ...
          wx.showToast({ title: err.message || '付款失败', icon: 'none' });
        }
      },
    });
  },
